                                             ...
                                             size_t snapshot_name_len)
{
    ...
    // snprintf(snapshot_name, snapshot_name_len, "%s/%s/%s-rr-snp", rr_path, rr_name, rr_name);

}

static inline void rr_get_snapshot_file_name_for_replay(char* rr_name, char* rr_path,
                                             char* snapshot_name,
                                             size_t snapshot_name_len)
{
    rr_assert(rr_name != NULL);
                                               ...
                                               size_t file_name_len)
{
    ...
    // snprintf(file_name, file_name_len, "%s/%s/%s-rr-nondet.log", rr_path, rr_name, rr_name);    
}

static inline void rr_get_nondet_log_file_name_for_replay(char* rr_name, char* rr_path,
                                               char* file_name,
                                               size_t file_name_len)
{
    rr_assert(rr_name != NULL && rr_path != NULL);
...
int rr_do_begin_replay(const char* file_name_full, CPUState* cpu_state)
{
    ...
    // rr_get_snapshot_file_name(rr_name, rr_path, name_buf, sizeof(name_buf));
    rr_get_snapshot_file_name_for_replay(rr_name, rr_path, name_buf, sizeof(name_buf));
    if (rr_debug_whisper()) {
        qemu_log("reading snapshot:\t%s\n", name_buf);
    }
    printf("loading snapshot\n");
    // printf("(debug) file_name_full : %s\n", file_name_full);
    // printf("(debug) rr_name : %s\n", rr_name);
    // printf("(debug) rr_path : %s\n", rr_path);
    // printf("(debug) name_buf : %s\n", name_buf);
    QIOChannelFile* ioc =
    ...
    // rr_get_nondet_log_file_name(rr_name, rr_path, name_buf, sizeof(name_buf));
    rr_get_nondet_log_file_name_for_replay(rr_name, rr_path, name_buf, sizeof(name_buf));
