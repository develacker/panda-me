...
void init_capstone(CPUArchState* env) 
{
    cs_arch arch;
    cs_mode mode;
#ifdef TARGET_I386
...
static int after_block_translate(CPUArchState *env, TranslationBlock *tb) 
{
    ...
    if (asid && panda_current_asid((CPUState *)env) != asid) return 0;
    ...
    panda_virtual_memory_rw((CPUState *)env, tb->pc, mem, tb->size, false);
...
bool init_plugin(void *self) {
    ...
    panda_arg_list *args = panda_get_args("crapr_list_opcodes");
    const char *name = panda_parse_string(args, "name", "crapr_list_opcodes");
