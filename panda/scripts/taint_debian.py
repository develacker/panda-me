...
def progress(msg):
    print Fore.GREEN + '[taint_debian.py] ' + Fore.RESET + Style.BRIGHT + msg + Style.RESET_ALL
    print
...
progress("First pass replay to figure out when to turn on taint (after file opened)\n")

logging.warn("Now adding args")
...
if stdin:
    ...
    # panda_args.extend(["-panda", "file_taint:filename=%s" % fileinput])
...
progress("Second pass replay to actually perform taint analysis\n")
print(insn)
